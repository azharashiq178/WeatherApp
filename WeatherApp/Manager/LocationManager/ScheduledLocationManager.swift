...
public class ScheduledLocationManager : NSObject {
    ...
    override init() {
        ...
        notifCenter.addObserver(self, selector: #selector(applicationDidEnterBackground), name: .NSExtensionHostDidEnterBackground, object: nil)
        notifCenter.addObserver(self, selector: #selector(applicationDidBecomeActive), name: .NSExtensionHostDidBecomeActive, object: nil)
...
//        locationManager.allowsBackgroundLocationUpdates = true
    ...
    public func stopGettingUserLocation() {
        ...
        notifCenter.removeObserver(self, name: .NSExtensionHostDidEnterBackground, object: nil)
        notifCenter.removeObserver(self, name: .NSExtensionHostDidBecomeActive, object: nil)
    ...
    fileprivate func startWaitForLocationUpdatesTimer() {
        ...
        waitForLocationUpdatesTimer = Timer.scheduledTimer(withTimeInterval: TimeInterval(kTimeToGetLocations), repeats: false, block: { timer in
            ...
            if self.application.applicationState == .background || self.application.applicationState == .inactive && self.bgTask == UIBackgroundTaskIdentifier.invalid {
